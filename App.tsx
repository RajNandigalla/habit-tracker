...
import { ListTodo, BookText, Settings2, Activity, X, BarChartBigIcon, Swords, Trophy } from 'lucide-react';
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <aside className="hidden md:flex w-72 flex-col m-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none z-20 h-fit max-h-[calc(100vh-6rem)] overflow-hidden shrink-0 overflow-y-auto no-scrollbar">
            ...
            <div className="p-4 mx-4 mb-4 bg-slate-50 dark:bg-slate-950/50 rounded-xl border border-slate-100 dark:border-slate-800">
              ...
              <p className="text-xs text-slate-700 dark:text-slate-300 italic font-serif leading-relaxed opacity-90">"Consistency is what transforms average into excellence."</p>
            </div>
        </aside>
          ...
          <div className="flex flex-col h-full bg-white dark:bg-slate-950">
             <div className="px-4 py-3 flex items-center justify-between border-b border-slate-100 dark:border-slate-800">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-lg font-extrabold text-slate-900 dark:text-white">TickOff</span>
                </div>
                <button onClick={closeSideMenu} className="p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full">
                  <X className="h-5 w-5" />
                </button>
             </div>
             ...
             <nav className="flex-1 px-4 space-y-2 mt-6">
                ...
                <NavItem to="/challenges" icon={<Swords />} label="Challenges" onClick={closeSideMenu} />
                <NavItem to="/achievements" icon={<Trophy />} label="Achievements" onClick={closeSideMenu} />
                <NavItem to="/journal" icon={<BookText />} label="Journal" onClick={closeSideMenu} />
                <NavItem to="/progress" icon={<BarChartBigIcon />} label="Progress" onClick={closeSideMenu} />
                <NavItem to="/settings" icon={<Settings2 />} label="Settings" onClick={closeSideMenu} />
             </nav>

             <div className="p-5 mt-auto border-t border-slate-100 dark:border-slate-800">
               <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl">
                 <p className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-2">Daily Quote</p>
                 <p className="text-sm text-slate-600 dark:text-slate-300 italic font-serif leading-relaxed">"Consistency is what transforms average into excellence."</p>
               </div>
             </div>
          </div>
        </SideMenu>
