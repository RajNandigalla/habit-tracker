...
import { GlobalStats, HeatmapData, MoodCorrelationData } from '../components/modules/ProgressModules';
...
export const Progress: React.FC = () => {
    ...
    const stats: GlobalStats = useMemo(() => {
        ...
        const completionRate = possibleCompletions > 0 
            ? Math.round((completionsLast30 / possibleCompletions) * 100) 
            : 0;
    ...
    const moodCorrelationData: MoodCorrelationData[] = useMemo(() => {
        const moodStats: Record<string, { totalPossible: number; completed: number; dayCount: number }> = {};
        ...
        for(let i=0; i<30; i++) {
            ...
            if (mood) {
                ...
                const activeHabitsCount = habits.filter(h => dayjs(h.createdAt).isBefore(dayjs(date).add(1, 'day'))).length;
        ...
        return Object.entries(moodStats).map(([mood, stats]) => ({
            ...
            completionRate: stats.totalPossible > 0 ? Math.round((stats.completed / stats.totalPossible) * 100) : 0
        }));
