...
const Toast: React.FC<{ toast: ToastMessage; onDismiss: (id: number) => void }> = ({ toast, onDismiss }) => {
    ...
    const transitionClasses = (isActive && !isExiting)
        // On-screen state
    ...
    return (
        ...
        >
            ...
            <p className="flex-grow text-base font-medium text-slate-800 dark:text-slate-200">{toast.message}</p>
            <button onClick={handleDismiss} className="flex-shrink-0 p-1 -m-1 rounded-full text-slate-400 hover:bg-black/10 dark:hover:bg-white/10" aria-label="Dismiss">
