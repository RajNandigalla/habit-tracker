...
export const DailyOverview: React.FC<DailyOverviewProps> = ({ 
  ...
  const moodOptions = [
    { value: 'motivated', icon: Zap, label: 'Motivated', color: 'text-amber-500', bg: 'bg-amber-100 dark:bg-amber-900/30' },
    { value: 'happy', icon: Smile, label: 'Great', color: 'text-green-500', bg: 'bg-green-100 dark:bg-green-900/30' },
    { value: 'neutral', icon: Meh, label: 'Okay', color: 'text-blue-500', bg: 'bg-blue-100 dark:bg-blue-900/30' },
    { value: 'tired', icon: Moon, label: 'Tired', color: 'text-purple-500', bg: 'bg-purple-100 dark:bg-purple-900/30' },
    { value: 'sad', icon: Frown, label: 'Down', color: 'text-slate-500', bg: 'bg-slate-100 dark:bg-slate-800' },
  ];
            ...
            <div className="relative z-10 flex items-center justify-between p-3.5 h-[88px]">
                ...
                <div className="relative w-[52px] h-[52px] flex-shrink-0">
                    <svg className="w-full h-full -rotate-90">
                        <circle cx="26" cy="26" r="23" className="stroke-white/20" strokeWidth="4" fill="none" />
                        <circle 
                            cx="26" cy="26" r="23" 
            ...
            <div className="relative z-10 flex items-center justify-between">
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div className="flex gap-6 text-right">
                        ...
                        <div>
                            <div className="text-xl font-bold leading-none">{totalHabits - completedHabits}</div>
             ...
             <div className="flex items-center justify-between">
                 ...
                 <div className="flex items-center gap-5">
                     ...
                     <div>
                         ...
                         <p className="text-sm text-slate-500 dark:text-slate-400">How are you feeling right now?</p>
                     </div>
                     ...
                     <div className="hidden md:flex items-center gap-3 px-4 py-2 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700 ml-4">
                        ...
                        <div>
                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-0.5">Current Streak</div>
                            <div className="text-sm font-bold text-slate-900 dark:text-white leading-none">{bestStreak} Days</div>
                        </div>
                     </div>
                 </div>
                 ...
                 {currentMood ? (
                     <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800 px-5 py-2.5 rounded-xl border border-slate-100 dark:border-slate-700 animate-fade-in">
                          ...
                          <div className={cn("flex items-center gap-2 font-bold px-2 py-0.5 rounded-md text-sm", 
                               moodOptions.find(m => m.value === currentMood)?.color,
                               moodOptions.find(m => m.value === currentMood)?.bg
                          )}>
                              {React.createElement(moodOptions.find(m => m.value === currentMood)?.icon || Smile, { className: "w-4 h-4" })}
                              <span className="capitalize">{currentMood}</span>
                          </div>
                          <button onClick={() => onLogMood(null as any)} className="ml-2 text-xs text-slate-400 underline hover:text-indigo-500">Edit</button>
                     </div>
                 ) : (
                           ...
                           >
                               <mood.icon className={cn(
                                   "w-6 h-6 transition-colors duration-200 text-slate-400 dark:text-slate-500",
                                    ...
                                    mood.value === 'sad' && "group-hover:text-slate-600 dark:group-hover:text-slate-300",
                               )} />
