...
import { cn, getTodayISO, generateId, calculateHabitStats, isHabitScheduledForDate } from '../../utils';
import { Check, Flame, Edit3, Timer, Crown, Trash2, BarChart2, Plus, AlertTriangle, ChevronLeft, ChevronRight, Calendar as CalendarIcon, Bell, Repeat, ShieldBan, Skull, AlertCircle } from 'lucide-react';
            ...
            days.push(
                <div key={date} className="flex items-center justify-center aspect-square">
                    <div className={cn(
                        ...
                        isToday && !isCompleted && "ring-2 ring-indigo-500 text-indigo-600 font-extrabold bg-indigo-50 dark:bg-indigo-900/20"
                    )}
            ...
            {showDeleteConfirm ? (
                 <div className="space-y-6 animate-fade-in">
                     <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-xl flex gap-4 items-start border border-red-100 dark:border-red-800/50">
                        ...
                        <div>
                            <h4 className="font-bold text-red-900 dark:text-red-100 mb-1">Are you absolutely sure?</h4>
                            <p className="text-sm text-red-700 dark:text-red-300 leading-relaxed">
                                This will permanently delete <strong>{habit.name}</strong> and remove all your progress history. This action cannot be undone.
                            </p>
                        </div>
                    </div>
                    <div className="flex justify-end gap-3 pt-2">
                        <Button variant="ghost" onClick={handleCancelDelete}>Cancel</Button>
                        <Button variant="danger" onClick={confirmDelete}>Yes, Delete It</Button>
                    </div>
                 </div>
            ) : !isEditing ? (
                     ...
                     <div>
                         <div className="flex items-start justify-between mb-2">
                             ...
                             <div className="flex gap-1">
                                {onFocus && habit.habitType === 'positive' && (
                                    <Button variant="ghost" size="icon-sm" onClick={handleStartFocus} title="Start Focus Timer">
                                        <Timer className="h-4 w-4 text-indigo-600 dark:text-indigo-400" />
                                    </Button>
                                )}
                                <Button variant="ghost" size="icon-sm" onClick={() => setIsEditing(true)} title="Edit Habit">
                                    <Edit3 className="h-4 w-4" />
                                </Button>
                                <Button variant="danger-ghost" size="icon-sm" onClick={() => setShowDeleteConfirm(true)} title="Delete Habit">
                                    <Trash2 className="h-4 w-4" />
                                </Button>
                             </div>
                         </div>
                         {habit.description && <p className="text-slate-500 dark:text-slate-400 leading-relaxed">{habit.description}</p>}
                         ...
                         {habit.challengeId && habit.challengeDuration && (
                             <div className="mt-3 inline-flex items-center gap-2 px-3 py-1 bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400 rounded-lg text-sm font-semibold border border-yellow-200 dark:border-yellow-800/50">
                                 ...
                                 Day {Math.min(habit.streak, habit.challengeDuration)} of {habit.challengeDuration} Challenge
                             </div>
                         )}
                     </div>
                         ...
                         <div className="md:col-span-7 space-y-6">
                             ...
                             <div className="grid grid-cols-3 gap-3">
                                 <div className="flex flex-col items-center justify-center p-4 bg-orange-50 dark:bg-orange-900/10 rounded-2xl">
                                     <div className="text-3xl font-extrabold text-orange-500 dark:text-orange-400 mb-1">{habit.streak}</div>
                                     <div className="text-[10px] font-bold text-orange-400/70 dark:text-orange-300 uppercase tracking-widest">{habit.habitType === 'negative' ? 'Days Free' : 'Current'}</div>
                                 </div>
                                 {habit.habitType === 'positive' && (
                                    <div className="flex flex-col items-center justify-center p-4 bg-blue-50 dark:bg-blue-900/10 rounded-2xl">
                                        <div className="text-3xl font-extrabold text-blue-500 dark:text-blue-400 mb-1">{stats.longestStreak}</div>
                                        <div className="text-[10px] font-bold text-blue-400/70 dark:text-blue-300 uppercase tracking-widest">Longest</div>
                                    </div>
                                 )}
                                 <div className="flex flex-col items-center justify-center p-4 bg-emerald-50 dark:bg-emerald-900/10 rounded-2xl">
                                     <div className="text-3xl font-extrabold text-emerald-500 dark:text-emerald-400 mb-1">{stats.totalCompletions}</div>
                                     <div className="text-[10px] font-bold text-emerald-400/70 dark:text-emerald-300 uppercase tracking-widest">{habit.habitType === 'negative' ? 'Incidents' : 'Total'}</div>
                                 </div>
                             </div>
                         ...
                         <div className="md:col-span-5 md:pl-6 pt-4 md:pt-0 border-t md:border-t-0 md:border-l border-slate-100 dark:border-slate-800 h-full">
                             <div className="flex items-center justify-between mb-4">
                                 ...
                                 <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 rounded-lg p-1">
                                    <button onClick={() => setViewDate(d => d.subtract(1, 'month'))} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded-md shadow-sm transition-all text-slate-600 dark:text-slate-300">
                                        <ChevronLeft className="h-4 w-4" />
                                    </button>
                                    <span className="text-xs font-bold w-24 text-center text-slate-700 dark:text-slate-200 select-none">
                                        {viewDate.format('MMMM YYYY')}
                                    </span>
                                    <button onClick={() => setViewDate(d => d.add(1, 'month'))} className="p-1.5 hover:bg-white dark:hover:bg-slate-700 rounded-md shadow-sm transition-all text-slate-600 dark:text-slate-300">
                 ...
                 <div className="space-y-4 animate-fade-scale">
                     ...
                     <div>
                        ...
                        <div className="flex items-center gap-3 h-[42px]">
                            {['#6366f1', '#ef4444', '#f59e0b', '#10b981', '#ec4899', '#8b5cf6', '#3b82f6'].map(c => (
                            <button
                                key={c}
                                type="button"
                                ...
                                editColor === c ? "ring-slate-400 scale-125" : "ring-transparent md:hover:scale-110"
                                )}
                    ...
                    <div className="flex gap-2 pt-4">
                        <Button variant="danger-ghost" onClick={() => setShowDeleteConfirm(true)} className="mr-auto">Delete</Button>
...
export const HabitListItem: React.FC<{ habit: Habit } & HabitActionProps> = ({ habit, toggleHabitCompletion, onFocus }) => {
  ...
  const getWeeklyProgress = () => {
      ...
      const count = habit.completedDates.filter(d => {
          const dObj = dayjs(d);
          return dObj.isAfter(startOfWeek.subtract(1, 'second')) && dObj.isBefore(endOfWeek.add(1, 'second'));
      }).length;
    ...
    <>
        <div 
            ...
            className={cn(
                ...
                isCompletedToday 
                    ? (isNegative ? "border-red-200 bg-red-50 dark:bg-red-900/10 dark:border-red-800/50" : "border-slate-200 dark:border-slate-700/50 bg-slate-50/80 dark:bg-slate-900/30")
        ...
        >
            ...
            <div className="flex-shrink-0 mr-3">
                ...
                >
                    {isNegative ? (
                         isCompletedToday ? <AlertCircle className="w-4 h-4" /> : <Skull className="w-3.5 h-3.5 text-slate-300 dark:text-slate-600" />
                    ) : (
                        <Check className={cn("w-4 h-4 transition-transform duration-300 ease-spring", isCompletedToday ? "scale-100" : "scale-0")} strokeWidth={3} />
                    )}
                </button>
            </div>
            ...
            <div className={cn("flex-grow min-w-0")}>
                <div className="flex items-center gap-2 mb-1">
                    <h3 className={cn(
                        ...
                        isCompletedToday ? "text-slate-500 dark:text-slate-400" : "text-slate-900 dark:text-white",
                        isNegative && isCompletedToday && "text-red-600 dark:text-red-400 decoration-red-300"
                    )}>
                        {habit.name}
                    </h3>
                    ...
                    {/* Visual Badges */}
                    <div 
                        className={cn("w-2 h-2 rounded-full flex-shrink-0 transition-all", isCompletedToday ? "opacity-50 grayscale" : "")}
                    ...
                    {!isScheduledToday && !isCompletedToday && habit.frequency !== 'weekly' && (
                        <span className="text-[10px] uppercase font-bold bg-slate-100 dark:bg-slate-800 text-slate-400 px-1.5 rounded">Rest Day</span>
                    )}
                </div>
                ...
                <div className="flex items-center gap-4 text-xs font-semibold text-slate-500 dark:text-slate-400">
                    <span className={cn("flex items-center gap-1", isCompletedToday && "opacity-75")}>
                        {isNegative ? (
                             <>
                                <ShieldBan className={cn("w-3.5 h-3.5", isCompletedToday ? "text-red-500" : "text-green-500")} />
                                {habit.streak} days clean
                             </>
                        ) : (
                             <>
                                <Flame className={cn("w-3.5 h-3.5 transition-colors", habit.streak > 0 && !isCompletedToday ? "text-orange-500 fill-orange-500" : "text-slate-400")} />
                                {habit.streak} streak
                             </>
                        )}
                    </span>
                    ...
                    <span className={cn(
                        ...
                        isCompletedToday ? "bg-slate-100 dark:bg-slate-800 text-slate-400" : "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300"
                    )}>
                        {habit.category}
                    </span>
                </div>
            ...
            <div className="hidden md:flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 relative z-20">
                 {!isNegative && (
                    <Button size="icon-sm" variant="ghost" onClick={handleFocus} title="Start Focus Timer" className="hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30">
                        <Timer className="h-4 w-4" />
                    </Button>
                 )}
                 <Button size="icon-sm" variant="ghost" onClick={handleStats} title="View Stats" className="hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
         <div className="space-y-4">
             ...
             <div className="space-y-3">
                 <label className="block text-base font-medium text-slate-700 dark:text-slate-300">Frequency</label>
             ...
             <div>
                <label className="mb-3 block text-sm font-medium text-slate-700 dark:text-slate-300">Color</label>
                    ...
                    {colors.map(c => (
                        <button
                            ...
                            className={cn(
                                ...
                                color === c ? "ring-2 ring-offset-2 ring-slate-400 scale-110" : "ring-transparent md:hover:scale-110"
                            )}
                            style={{ backgroundColor: c }}
                        />
                    ))}
         ...
         <div className="flex justify-end gap-2 pt-2">
             ...
             <Button type="submit" disabled={!name} leftIcon={<Plus className="w-4 h-4"/>}>Create Habit</Button>
         </div>
      </form>
    </Modal>
  );
