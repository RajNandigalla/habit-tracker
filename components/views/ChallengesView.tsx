                ...
                <div className="max-w-7xl mx-auto px-4 py-6 md:px-8 pb-24 md:pb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {CHALLENGES.map((challenge, index) => {
                            ...
                            return (
                                <div key={challenge.id} className="h-full animate-slide-up" style={{ animationDelay: `${index * 50}ms`, opacity: 0 }}>
                                    <Card 
                                        ...
                                        {isJoined && activeHabit && (
                                            <div className="absolute bottom-0 left-0 w-full h-1 bg-slate-100 dark:bg-slate-800">
                                                <div 
                                                    ...
                                                    style={{ width: `${Math.min((activeHabit.streak / challenge.durationDays) * 100, 100)}%` }}
                                                />
                                            </div>
                                        )}
                                    </Card>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-start gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700 animate-fade-scale">
                            <div className="p-3 bg-white dark:bg-slate-800 rounded-lg shadow-sm">
                                <selectedChallenge.icon className="h-8 w-8" style={{ color: selectedChallenge.color }} />
                            </div>
                            <div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-3 border border-slate-100 dark:border-slate-700 rounded-lg text-center">
                                ...
                                <div className="text-xl font-bold text-slate-900 dark:text-white">{selectedChallenge.durationDays} Days</div>
                            </div>
                            <div className="p-3 border border-slate-100 dark:border-slate-700 rounded-lg text-center">
                        ...
                        {!getActiveHabitForChallenge(selectedChallenge.id) && (
                            <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl p-4">
                                <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-3">How do you want to track this?</h4>
                                ...
                                <div className="flex flex-col gap-3">
                                    ...
                                    >
                                        <div className={cn(
                                            ...
                                            {linkMode === 'existing' && <div className="w-2.5 h-2.5 rounded-full bg-indigo-500" />}
                                        </div>
                                        <div className="flex-1">
                                            <div className="font-semibold text-sm text-slate-900 dark:text-white flex items-center gap-2">
                        ...
                        <div className="pt-4 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-3">
                            <Button variant="ghost" onClick={() => setSelectedChallenge(null)}>Close</Button>
                            {getActiveHabitForChallenge(selectedChallenge.id) ? (
                                <Button disabled className="opacity-50 cursor-not-allowed">Already Joined</Button>
                            ) : (
                                <Button 
                                    onClick={handleJoin} 
