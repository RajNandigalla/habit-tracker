         ...
         <div className="max-w-7xl mx-auto px-4 py-6 md:px-8 pb-24 md:pb-8">
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-4 gap-4">
                ...
                <div className="hidden md:flex items-center gap-3">
                    <Button onClick={() => setModalOpen(true)} size="sm" className="shadow-lg shadow-indigo-500/20 dark:shadow-indigo-900/20 h-10">
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center animate-enter">
                <div className="relative mb-8 group">
                    ...
                    <div className="relative w-24 h-24 bg-gradient-to-br from-indigo-50 to-white dark:from-slate-800 dark:to-slate-900 rounded-3xl flex items-center justify-center shadow-xl shadow-indigo-500/10 dark:shadow-indigo-900/10 border border-indigo-100 dark:border-slate-700 animate-bounce" style={{ animationDuration: '3s' }}>
                ...
                <p className="text-slate-500 dark:text-slate-400 max-w-sm mb-8 leading-relaxed text-base">
                    Your journey to success starts with a single step. Add your first habit to begin tracking your potential.
                </p>
                
                <Button onClick={() => setModalOpen(true)} size="lg" className="hidden md:inline-flex shadow-xl shadow-indigo-600/20 dark:shadow-indigo-900/20 px-8">
                    Create Habit
                </Button>
            </div>
            ) : (
                ...
                {filteredHabits.length === 0 ? (
                    <div className="text-center py-12 text-slate-500 dark:text-slate-400 animate-fade-scale">
                        No habits found in <span className="font-semibold text-indigo-600 dark:text-indigo-400">{selectedCategory}</span>.
                    </div>
                ) : (
                    <div className="space-y-3">
                        {filteredHabits.map((habit, index) => (
                            <div key={habit.id} className="animate-slide-up" style={{ animationDelay: `${index * 50}ms` }}>
                                <HabitListItem 
                                    habit={habit} 
