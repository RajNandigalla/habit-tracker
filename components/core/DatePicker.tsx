...
const YearView: React.FC<{
    ...
    useEffect(() => {
        const yearToScroll = (selectedYearValue && selectedYearValue >= startYear && selectedYearValue < startYear + 100) 
            ? selectedYearValue 
            : currentNavYear;
    ...
    return (
        <div ref={yearGridRef} className="grid grid-cols-4 gap-y-1 gap-x-2 py-2 h-[220px] overflow-y-auto pr-2">
            {years.map(year => {
                ...
                return (
                    <button
                        ...
                        className={`p-2 rounded-md text-base text-center 
                            ${isSelected ? 'bg-indigo-600 text-white font-semibold hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600' : 
    ...
    return (
        <>
            <div className="flex items-center justify-between mb-2">
                <Button type="button" variant="ghost" size="icon-sm" onClick={handlePrev} className="rounded-full"><ChevronLeftIcon className="w-5 h-5" /></Button>
                 <div className="flex-grow text-center">
                    <Button type="button" variant="ghost" onClick={handleHeaderClick} className="font-semibold text-slate-800 dark:text-slate-100 hover:text-indigo-600 dark:hover:text-indigo-400 text-base">
                        {headerTitle}
                    </Button>
                </div>
                <Button type="button" variant="ghost" size="icon-sm" onClick={handleNext} className="rounded-full"><ChevronRightIcon className="w-5 h-5" /></Button>
            </div>
            ...
            <div className="min-h-[220px] overflow-hidden">
                ...
                {currentView === 'year' && (
                    <div
                        key={Math.floor(viewDate.getFullYear() / 100)}
                        className={animationClass}
                    >
                        <YearView viewDate={viewDate} stagedValue={stagedValue} handleYearClick={handleYearClick} />
                    </div>
                )}
            </div>
            ...
            {showActions && (
                <div className="flex justify-between items-center pt-3 mt-2 border-t border-slate-200 dark:border-slate-700">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button variant="primary" size="sm" onClick={handleSet} className='px-6 py-2'>Set</Button>
                    </div>
                </div>
            )}
        </>
...
export const DatePicker: React.FC<DatePickerProps> = ({ id = 'date-picker', value, onChange, disabled = false }) => {
    ...
    const updatePosition = useCallback(() => {
        ...
        computePosition(buttonRef.current, dropdownRef.current, {
            ...
            setDropdownStyles({
                position: strategy, left: `${x}px`, top: `${y}px`,
    ...
    const renderDayView = useCallback(() => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            const isSelected = stagedDateObj?.getFullYear() === currentYear && stagedDateObj?.getMonth() === currentMonth && stagedDateObj?.getDate() === day;
            ...
            grid.push(
                <button
                    ...
                    className={`w-9 h-9 rounded-full flex items-center justify-center text-base font-semibold transition-colors
                        ${isSelected ? 'bg-indigo-600 text-white hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600' : 
                        isToday ? 'bg-slate-200 dark:bg-slate-600 text-slate-900 dark:text-white hover:bg-slate-300 dark:hover:bg-slate-500' : 
        ...
        <div className="relative">
            ...
            {isMobile ? (
                ...
                isOpen && ReactDOM.createPortal(
                     <div ref={dropdownRef} style={dropdownStyles} className="z-50 bg-white dark:bg-slate-800 rounded-lg shadow-2xl border border-slate-200 dark:border-slate-700 p-4 transition-opacity duration-150">
                        <DatePickerContent
                            viewDate={viewDate}
                            stagedValue={stagedValue}
                            currentView={currentView}
