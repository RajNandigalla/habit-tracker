...
const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({ onClick, isLoading = false, fullWidth = false, className = '', children, ...props }) => {
  const baseClasses = 'relative inline-flex items-center justify-center gap-4 px-4 py-2 border rounded-md shadow-sm text-base font-semibold transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 focus:ring-offset-white dark:focus:ring-offset-slate-900 disabled:opacity-60 disabled:cursor-not-allowed';
  
  // Light theme uses white bg, light gray border. Dark theme uses dark bg, slightly lighter border to match Google's branding.
  const themeClasses = 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-700';
  ...
  return (
    ...
    >
      ...
      <span className={`flex items-center justify-center gap-3 transition-opacity ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
