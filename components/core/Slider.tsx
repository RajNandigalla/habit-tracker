...
export const Slider: React.FC<SliderProps> = ({
  ...
  return (
    <div className={cn("w-full select-none touch-none", disabled && "opacity-50 pointer-events-none", className)}>
      {/* Header Label & Value */}
      {(label || showValue) && (
      ...
      <div className="flex items-center gap-4">
        {leftIcon && <div className={cn("transition-colors duration-300", isDragging ? "text-indigo-500" : "text-slate-400")}>{leftIcon}</div>}
        ...
        >
            ...
            >
                ...
                <div className={cn(
                    ...
                    isDragging ? "scale-125 border-indigo-600 bg-indigo-50 dark:bg-indigo-900" : isHovered ? "scale-110" : "scale-100"
                )}>
                    {/* Inner dot */}
                    <div className={cn(
        ...
        {rightIcon && <div className={cn("transition-colors duration-300", isDragging ? "text-indigo-500" : "text-slate-400")}>{rightIcon}</div>}
      </div>
    </div>
  );
};
