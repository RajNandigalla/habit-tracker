...
import { 
    Zap, Activity, Rocket, Star, Target, Award, Shield, CheckCircle2, Crown, Flame, Gem, Trophy, Sun, Moon,
    Heart, Briefcase, Brain, Leaf, Dumbbell, Layers, BookOpen, Camera, Flag, Cloud, Calendar
} from 'lucide-react';
import dayjs from 'dayjs';

export const useAchievements = (habits: Habit[], journalEntries: JournalEntry[], stats: { totalCompletions: number, longestStreak: number }) => {
    return useMemo(() => {
        ...
        // --- 1. Milestone Levels (Total Checks) - 15 Levels ---
        const checkMilestones = [
            { count: 1, title: 'First Step', icon: Zap, color: 'text-yellow-500 bg-yellow-100 dark:bg-yellow-900/20' },
            { count: 10, title: 'Rolling', icon: Activity, color: 'text-blue-500 bg-blue-100 dark:bg-blue-900/20' },
            { count: 25, title: 'Momentum', icon: Rocket, color: 'text-indigo-500 bg-indigo-100 dark:bg-indigo-900/20' },
            { count: 50, title: 'Half Century', icon: Star, color: 'text-purple-500 bg-purple-100 dark:bg-purple-900/20' },
            { count: 100, title: 'Century Club', icon: Target, color: 'text-red-500 bg-red-100 dark:bg-red-900/20' },
            { count: 250, title: 'Dedicated', icon: Award, color: 'text-orange-500 bg-orange-100 dark:bg-orange-900/20' },
            { count: 500, title: 'Committed', icon: Shield, color: 'text-green-500 bg-green-100 dark:bg-green-900/20' },
            { count: 750, title: 'Disciplined', icon: CheckCircle2, color: 'text-teal-500 bg-teal-100 dark:bg-teal-900/20' },
            { count: 1000, title: 'Grandmaster', icon: Crown, color: 'text-amber-500 bg-amber-100 dark:bg-amber-900/20' },
            { count: 1500, title: 'Unstoppable', icon: Flame, color: 'text-rose-500 bg-rose-100 dark:bg-rose-900/20' },
            { count: 2000, title: 'Virtuoso', icon: Gem, color: 'text-fuchsia-500 bg-fuchsia-100 dark:bg-fuchsia-900/20' },
            { count: 3000, title: 'Titan', icon: Trophy, color: 'text-slate-700 bg-slate-200 dark:text-slate-200 dark:bg-slate-700' },
            { count: 5000, title: 'Legend', icon: Sun, color: 'text-yellow-600 bg-yellow-200 dark:text-yellow-300 dark:bg-yellow-800' },
            { count: 7500, title: 'Mythic', icon: Moon, color: 'text-violet-600 bg-violet-200 dark:text-violet-300 dark:bg-violet-800' },
            { count: 10000, title: 'Godlike', icon: Zap, color: 'text-cyan-600 bg-cyan-200 dark:text-cyan-300 dark:bg-cyan-800' },
        ];

        checkMilestones.forEach(m => {
            list.push({
        ...
        const categoryColors: Record<string, string> = {
            ...
            [HabitCategory.LEARNING]: 'text-purple-600 bg-purple-100 dark:bg-purple-900/20 dark:text-purple-400',
            [HabitCategory.MINDFULNESS]: 'text-teal-600 bg-teal-100 dark:bg-teal-900/20 dark:text-teal-400',
            [HabitCategory.FITNESS]: 'text-orange-600 bg-orange-100 dark:bg-orange-900/20 dark:text-orange-400',
        ...
        list.push({
         ...
         const happyEntries = journalEntries.filter(j => j.mood === 'happy' || j.mood === 'motivated').length;
         list.push({
             id: 'optimist',
             title: 'Optimist',
